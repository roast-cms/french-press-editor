...
- **Much better than ContentEditable:** don't make your users suffer like that.
...
## Specs.
To see if this tool is right for you, please have a look through the list of specs that come with it below. Everything listed comes pre-packaged.
...
- Mobile-ready. Even for iOS: there's a smart design implementation for format hovering format menu that makes working on a mobile device easy. Android devices may still have bugs.
...
- **Placeholder text:** you can define grey text that users sees on an empty document.
...
#### User controls.
- **Floating edit menu:** à la Medium. When user selects text a menu just above the highlight area appears with options to _convert text block into a heading_, _convert text block into a quote_, _add link to selected text_, _make selected text bold_, _make selected text italic_.
...
- If the user pastes HTML form another web page or document, it will be converted and interpreted to follow the structure outlined in "content block types" and "text format options."

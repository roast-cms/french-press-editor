...
`french-press-editor` was built for fast installation and a pre-defined set of tools that make an editorial experience that has most of the required features of a typical blog or publication entry with the absolute minimum required to make it useable. This tool is extendable, via Slate's plugins infrastructure, however, it has some prerequisites and opinionated limitations. That's what makes it a straight-forward tool that can be quickly implemented into your codebase.
...
`callbackPropsUpdate` | `Function` | You can specify your own function here that will be placed inside `componentWillReceiveProps` inside the editor (provides `props` and `nextProps` objects).
